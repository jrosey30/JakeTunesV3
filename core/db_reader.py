...
def build_order_mhod(position=0):
    """Build a type-100 ordering mhod (44 bytes, used inside mhip).

    2026-07-24: the ordinal was NEVER WRITTEN — this emitted 44 bytes with the
    position field left zero, so every item in every playlist reported position
    0 (the master library list had N songs all claiming one slot). libgpod's
    write_playlist_mhips does `mhod.data.track_pos = i` with i counting from 0;
    the field sits at offset 0x18 of the mhod. This is the ONLY change applied
    on top of the known baseline — everything else in this file is the
    empirically-validated configuration this device actually wants.
    """
    rec = bytearray(44)
    struct.pack_into('<4s', rec, 0, b'mhod')
    ...
    struct.pack_into('<I', rec, 0x18, position)
    return bytes(rec)
