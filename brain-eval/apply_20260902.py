...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260902")
CAND = "/tmp/mood-index.candidate-20260902.bin"
EXPECT_LIVE_SHA = "e138d74cb3426c88e2511e864c8b55bf7ee6ad67"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 62057924 or int(emb_stat.st_mtime) != 1788329291:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260902.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9968:
