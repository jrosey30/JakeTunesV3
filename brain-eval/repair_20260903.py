#!/usr/bin/env python3
"""repair_20260903 — completes the 7th-clobber repair that the aborted 08-27 session never ran of the 08-18 mood-index repair after the
...
Writes: /tmp/mood-index.candidate-20260903.bin + repair_20260903_ids.json
Never touches the NAS. Apply is a separate, gated step.
"""
import json, os, struct, sys, time
sys.path.insert(0, "/Users/jakerosenbaumnas/.brain-eval-wt-20260903/brain-eval")
os.environ.setdefault("JT_STATE_DIR", "/tmp/brain-snap-20260903")
import numpy as np
import run_eval as R
import urllib.request

HERE = "/Users/jakerosenbaumnas/.brain-eval-wt-20260903/brain-eval"
SNAP = "/tmp/brain-snap-20260903"
CAND = "/tmp/mood-index.candidate-20260903.bin"
FRESH_CUTOFF = "2026-09-03T05:55"  # tonight's trainer (launchd, clean 06:00:06Z-06:01:45Z)
...
def main():
    ...
    # ---- 1. fidelity gate: HYBRID (2026-08-31) ----
    # Only +14 imports tonight -> fresh cohort < the 20 minimum. Per the 08-30
    # note, small-import nights use the 08-24 aged-cohort variant; tonight we
    # gate on BOTH: every trainer-fresh vector (validates the CURRENT trainer
    # incl. recent edits) topped up to 50 with aged never-repaired
    # trainer-authored vectors (desc at < 2026-08-16, never in any
    # repair_*_ids.json). Same >=0.98 / 95% bar on the combined cohort.
    import glob
    prior_ids = set()
    ...
    if len(fresh) < 50:
        ...
        if len(fresh) + len(aged) < 20:
            raise SystemExit(f"[abort] only {len(fresh)} fresh + {len(aged)} aged vectors to gate on (<20)")
        need = 50 - len(fresh)
    ...
    json.dump({"repaired": [i for i, _ in suspects], "pruned_orphans": orphans,
              ...
              open(os.path.join(HERE, "repair_20260903_ids.json"), "w"), indent=1)
