...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260822")
CAND = "/tmp/mood-index.candidate-20260822.bin"
EXPECT_LIVE_SHA = "f7b9949d77f42181fbe487d800a53940799b8d81"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260822.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9645:
