...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260829")
CAND = "/tmp/mood-index.candidate-20260829.bin"
EXPECT_LIVE_SHA = "24f5245aeae4402c0a3a561e08ce4bd9abe1a18e"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 61043504 or int(emb_stat.st_mtime) != 1787983300:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260829.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9806:
