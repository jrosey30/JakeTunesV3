...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260824")
CAND = "/tmp/mood-index.candidate-20260824.bin"
EXPECT_LIVE_SHA = "45ac3b10e6847afc429b6891dafed126b7894430"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 60188932 or int(emb_stat.st_mtime) != 1787523488:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260824.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9692:
