#!/usr/bin/env python3
"""repair_20260905 — nightly integrity SCAN (first import day with NO
whole-map replay: pre-check 12 orphans / 5 dup groups vs the classic
~125/60 signature; mood mtime == trainer-done 02:08). Run to VERIFY the
index matches intended state; repair/apply only if suspects prove real.
...
Writes: /tmp/mood-index.candidate-20260905.bin + repair_20260905_ids.json
Never touches the NAS. Apply is a separate, gated step.
"""
import json, os, struct, sys, time
sys.path.insert(0, "/Users/jakerosenbaumnas/.brain-eval-wt-20260905/brain-eval")
os.environ.setdefault("JT_STATE_DIR", "/tmp/brain-snap-20260905")
import numpy as np
import run_eval as R
import urllib.request

HERE = "/Users/jakerosenbaumnas/.brain-eval-wt-20260905/brain-eval"
SNAP = "/tmp/brain-snap-20260905"
CAND = "/tmp/mood-index.candidate-20260905.bin"
FRESH_CUTOFF = "2026-09-05T05:55"  # tonight's trainer (launchd, clean 06:00:04Z-06:08:04Z)
...
def main():
    ...
    # ---- 1. fidelity gate: AGED key-trusted (09-04 variant) ----
    # Tonight (2026-09-05) the fresh-50 cohort is UNUSABLE as a ruler for a
    # different reason than 09-04: no replay fired (mood mtime == trainer-done
    # 02:08), but library.json was re-pushed at 02:50 with post-trainer librosa
    # results — 33/50 fresh vectors were embedded before their bpm (and for
    # Bear Ghost, genre) landed. VERIFIED faithful under the trainer's actual
    # 02:00 view: 6/6 sampled misses hit cos 1.0000 with bpm/key nulled and
    # 3/3 Bear Ghost hit 1.0000 descriptor-only (/tmp/notempo_check_20260905,
    # /tmp/ablate_20260905). Gate the reconstruction on aged never-repaired
    # key-trusted te=2 vectors instead. Same >=0.98 / 95% bar.
    import glob
    prior_ids = set()
    ...
    fresh_pool = sorted(i for i in cur if str(i) in desc and (desc[str(i)].get("at") or "") >= FRESH_CUTOFF
                        and i in intended and i in lib_ids)
    print(f"trainer-fresh vectors tonight: {len(fresh_pool)} (NOT gated on — post-trainer bpm/genre arrival, see header)")
    ...
    for j, i in enumerate(fresh):
        if sims[j] < 0.98:
            t = by_id.get(i) or by_id.get(str(i)) or {}
    ...
    n_fresh_susp = sum(1 for i, _ in suspects if (desc.get(str(i), {}).get("at") or "") >= FRESH_CUTOFF)
    print(f"suspects (stored != intended, cos<0.98): {len(suspects)}  "
          f"[fresh-tonight metadata-skew: {n_fresh_susp}, aged (clobber-indicating): {len(suspects) - n_fresh_susp}]  "
    ...
    json.dump({"repaired": [i for i, _ in suspects], "pruned_orphans": orphans,
              ...
              open(os.path.join(HERE, "repair_20260905_ids.json"), "w"), indent=1)
