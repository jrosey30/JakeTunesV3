#!/usr/bin/env python3
"""repair_20260826 — 6th re-run of the 08-18 mood-index repair after the
day-sync RESURRECTED the pre-repair map on an import day (+91 tracks; see
REPORT-20260820-nightly.md for the root cause, PROPOSAL-mood-import-clobber.md
for the gated fix). Pre-check tonight: orphans=104 dup_groups=60 (healthy 0/1).
...
Writes: /tmp/mood-index.candidate-20260826.bin + repair_20260826_ids.json
Never touches the NAS. Apply is a separate, gated step.
"""
import json, os, struct, sys, time
sys.path.insert(0, "/tmp/brain-eval-worktree/brain-eval")
os.environ.setdefault("JT_STATE_DIR", "/tmp/brain-snap-20260826")
import numpy as np
import run_eval as R
import urllib.request

HERE = "/tmp/brain-eval-worktree/brain-eval"
SNAP = "/tmp/brain-snap-20260826"
CAND = "/tmp/mood-index.candidate-20260826.bin"
FRESH_CUTOFF = "2026-08-26T05:55"  # tonight's trainer run (UTC; clean finish 06:01Z)
...
def main():
    ...
    json.dump({"repaired": [i for i, _ in suspects], "pruned_orphans": orphans,
              ...
              open(os.path.join(HERE, "repair_20260826_ids.json"), "w"), indent=1)
