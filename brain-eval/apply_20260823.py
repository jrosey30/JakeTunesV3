...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260823")
CAND = "/tmp/mood-index.candidate-20260823.bin"
EXPECT_LIVE_SHA = "29c4bad53eb7c2840cb208ffabcbf2d750063db0"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260823.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9686:
