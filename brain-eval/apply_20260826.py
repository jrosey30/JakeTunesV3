...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260826")
CAND = "/tmp/mood-index.candidate-20260826.bin"
EXPECT_LIVE_SHA = "a723fe7c441d91c52a0503cbdc5099d4e0f6ff48"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 60791436 or int(emb_stat.st_mtime) != 1787724104:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260826.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9783:
