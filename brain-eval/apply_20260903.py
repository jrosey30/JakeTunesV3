...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260903")
CAND = "/tmp/mood-index.candidate-20260903.bin"
EXPECT_LIVE_SHA = "646db49a35d8e91bf9ca8e9512fc28ce84d71a2d"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 62168588 or int(emb_stat.st_mtime) != 1788415664:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260903.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9985:
