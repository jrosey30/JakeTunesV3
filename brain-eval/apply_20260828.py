...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260828")
CAND = "/tmp/mood-index.candidate-20260828.bin"
EXPECT_LIVE_SHA = "156f6db2170f86e30318ee5c8420a215936a2663"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 60871360 or int(emb_stat.st_mtime) != 1787896899:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260828.tmp"
