...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260827")
CAND = "/tmp/mood-index.candidate-20260827.bin"
EXPECT_LIVE_SHA = "14d998835293f6dcb68bda43900cb9b2ab2d354c"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 60871360 or int(emb_stat.st_mtime) != 1787814796:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260827.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9791:
