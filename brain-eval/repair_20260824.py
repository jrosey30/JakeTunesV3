#!/usr/bin/env python3
"""repair_20260824 — 5th re-run of the 08-18 mood-index repair after the
...
Writes: /tmp/mood-index.candidate-20260824.bin + repair_20260824_ids.json
Never touches the NAS. Apply is a separate, gated step.
"""
import json, os, struct, sys, time
sys.path.insert(0, "/tmp/brain-eval-worktree/brain-eval")
os.environ.setdefault("JT_STATE_DIR", "/tmp/brain-snap-20260824")
import numpy as np
import run_eval as R
import urllib.request

HERE = "/tmp/brain-eval-worktree/brain-eval"
SNAP = "/tmp/brain-snap-20260824"
CAND = "/tmp/mood-index.candidate-20260824.bin"
FRESH_CUTOFF = "2026-08-23T05:55"  # LAST SUCCESSFUL trainer (08-24 trainer FATAL ETIMEDOUT before any write; descriptors unchanged since 08-23 02:01)
...
def main():
    ...
    # ---- 1. fidelity gate — AGED-COHORT variant (2026-08-24) ----
    # Tonight the trainer FATAL'd before writing, so the usual trainer-fresh
    # cohort (desc at >= cutoff) was itself clobbered by the 18:18 day-sync
    # replay (measured: those 50 sit at cos 0.40-0.58 vs intended — they hold
    # import-time bare-genre embeds and belong in the SUSPECT set, not the
    # gate). Instead gate on trainer-authored vectors that SURVIVE in the
    # replayed stale map: tracks enriched well before 08-18 that were never
    # repaired/pruned/excluded in any prior recipe run (the 08-23 run verified
    # ~8.9k such tracks matched intended at >=0.98). Stratified deterministic
    # sample of 50 across the id range.
    prior_ids = set()
    for f in ("repair_20260818_ids.json", "repair_20260820_ids.json",
              "repair_20260822_ids.json", "repair_20260823_ids.json"):
        j = json.load(open(os.path.join(HERE, f)))
        for k2 in ("repaired", "pruned_orphans", "excluded_tempo_view"):
            prior_ids.update(int(x) for x in j.get(k2, []))
    ...
    if len(aged) < 200:
        raise SystemExit(f"[abort] only {len(aged)} aged never-suspect vectors to gate on (<200)")
    ...
    print(f"fidelity gate (aged cohort, {len(aged)} eligible): {ok}/{len(fresh)} "
          f"trainer-authored vectors match reconstruction >=0.98 (min {min(sims):.4f})")
    ...
    for i in ids_all:
        ...
        if c < 0.98:
            ...
            # trainer embedded (teb recorded as a REAL tempo, and library bpm
            # since nulled/moved — the 08-18 Stereolab case). teb==0 means the
            # trainer embedded WITHOUT tempo (import preceded bpm analysis);
            # the reconstruction now has the real bpm, i.e. a strictly BETTER
            # view — and tonight the trainer's own tempo catch-up (which
            # normally re-embeds these at 2:00) FATAL'd before running, so
            # excluding them would preserve clobber-distance import embeds
            # (measured cos 0.44-0.65, hijacking ret-007's top slots).
    ...
    json.dump({"repaired": [i for i, _ in suspects], "pruned_orphans": orphans,
              ...
              open(os.path.join(HERE, "repair_20260824_ids.json"), "w"), indent=1)
