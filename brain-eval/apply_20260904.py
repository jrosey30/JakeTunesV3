...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260904")
CAND = "/tmp/mood-index.candidate-20260904.bin"
EXPECT_LIVE_SHA = "15f4f8097c057d51e93ef68555e92367f1c56bb0"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 63914620 or int(emb_stat.st_mtime) != 1788502806:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260904.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 10262:
