...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260830")
CAND = "/tmp/mood-index.candidate-20260830.bin"
EXPECT_LIVE_SHA = "1a5457c57bdf1cbefdbc7e59b4b31c527c34f73f"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 61240240 or int(emb_stat.st_mtime) != 1788069678:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260830.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9838:
