...
BACKUP = os.path.join(NAS, "mood-index.bin.pre-repair-20260831")
CAND = "/tmp/mood-index.candidate-20260831.bin"
EXPECT_LIVE_SHA = "05da172d1b1b97e34f295ae435d76ca0e16ca069"
def sha(p):
    h = hashlib.sha1()
    with open(p, "rb") as f:
...
if emb_stat.st_size != 61326312 or int(emb_stat.st_mtime) != 1788156043:
...
cand_sha = sha(CAND)
tmp = f"{LIVE}.{os.getpid()}.repair20260831.tmp"
...
final = sha(LIVE)
if final != cand_sha or dim != 1536 or count != 9852:
