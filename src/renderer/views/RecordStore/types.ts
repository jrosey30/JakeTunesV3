// Music Man's Record Store — renderer-side type contracts.
//
// ⚠️ TWIN: src/main/record-store/types.ts — keep in sync. The renderer
// (web tsconfig, rootDir src/renderer) cannot import from src/main without
// breaking the composite-project boundary, so these IPC/disk-cache shapes
// are mirrored here, matching the established twin pattern used elsewhere
// in this codebase (e.g. albumReleaseDate). Dependency-free by design.
// If you change a shape on either side, update the other in the SAME commit.
